 ...
 *
 * <p>These govern hashing only. Verification takes its parameters from the PHC string it is checking,
 * which is what lets a test provision an Account cheaply and still reach the identical verification
 * path, and what lets production parameters be raised later without invalidating existing Accounts.
 *
...
public record Argon2Parameters(int memoryKib, int iterations, int parallelism, int outputLength) {
    ...
    public static final Argon2Parameters PRODUCTION =
            new Argon2Parameters(OWASP_MINIMUM_MEMORY_KIB, OWASP_MINIMUM_ITERATIONS, OWASP_MINIMUM_PARALLELISM, 32);

    public Argon2Parameters {
        if (memoryKib < 8 * parallelism) {
            throw new IllegalArgumentException("Argon2 needs at least 8 KiB of memory per lane, got " + memoryKib);
        }
        if (iterations < 1) {
        ...
        if (outputLength < 4) {
            throw new IllegalArgumentException("outputLength must be at least 4 bytes, got " + outputLength);
        }
    }
}
